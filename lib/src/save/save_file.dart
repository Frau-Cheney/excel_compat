...
class Save {
  ...
  void _addNewColumn(XmlElement columns, int min, int max, double width) {
    columns.children.add(XmlElement(XmlName.parts('col'), [
      XmlAttribute(XmlName.parts('min'), (min + 1).toString()),
      XmlAttribute(XmlName.parts('max'), (max + 1).toString()),
      XmlAttribute(XmlName.parts('width'), width.toStringAsFixed(2)),
      XmlAttribute(XmlName.parts('bestFit'), "1"),
      XmlAttribute(XmlName.parts('customWidth'), "1"),
    ], []));
  }
  ...
  XmlElement _createCell(String sheet, int columnIndex, int rowIndex,
    ...
    var attributes = <XmlAttribute>[
      XmlAttribute(XmlName.parts('r'), rC),
      if (value is TextCellValue) XmlAttribute(XmlName.parts('t'), 's'),
      if (value is BoolCellValue) XmlAttribute(XmlName.parts('t'), 'b'),
    ];
    ...
    if (_excel._styleChanges && cellStyle != null) {
      ...
      attributes.insert(
        1,
        XmlAttribute(XmlName.parts('s'), '$upperLevelPos'),
      );
      ...
      attributes.insert(
        1,
        XmlAttribute(
            XmlName.parts('s'), '${_excel._cellStyleReferenced[sheet]![rC]}'),
      );
    }
    ...
    switch (value) {
      ...
      case FormulaCellValue():
        children = [
          XmlElement(XmlName.parts('f'), [], [XmlText(value.formula)]),
          XmlElement(XmlName.parts('v'), [], [XmlText('')]),
        ];
      case IntCellValue():
        ...
        children = [
          XmlElement(XmlName.parts('v'), [], [XmlText(v)]),
        ];
      ...
      case TextCellValue():
        children = [
          XmlElement(XmlName.parts('v'), [], [
            XmlText(_excel._sharedStrings.indexOf(sharedString!).toString())
          ]),
        ];
      case BoolCellValue():
        children = [
          XmlElement(XmlName.parts('v'), [], [XmlText(value.value ? '1' : '0')]),
        ];
    }

    return XmlElement(XmlName.parts('c'), attributes, children);
  }

  /// Create a new row in the sheet.
  XmlElement _createNewRow(XmlElement table, int rowIndex, double? height) {
    var row = XmlElement(XmlName.parts('row'), [
      XmlAttribute(XmlName.parts('r'), (rowIndex + 1).toString()),
      if (height != null)
        XmlAttribute(XmlName.parts('ht'), height.toStringAsFixed(2)),
      if (height != null) XmlAttribute(XmlName.parts('customHeight'), '1'),
    ], []);
  ...
  void _processStylesFile() {
    ...
    _innerCellStyle.forEach((cellStyle) {
      ...
      if ((_excel._patternFill.contains(backgroundColor) ||
        ...
        attributes.add(XmlAttribute(XmlName.parts('applyFill'), '1'));
      }

      if (_fontStyleIndex(_excel._fontStyleList, _fs) != -1 &&
          _fontStyleIndex(innerFontStyle, _fs) != -1) {
        attributes.add(XmlAttribute(XmlName.parts('applyFont'), '1'));
      }
      ...
      if (horizontalAlign != HorizontalAlign.Left ||
          ...
          rotation != 0) {
        attributes.add(XmlAttribute(XmlName.parts('applyAlignment'), '1'));
        var childAttributes = <XmlAttribute>[];

        if (textWrapping != null) {
          childAttributes.add(XmlAttribute(
              XmlName.parts(textWrapping == TextWrapping.Clip
                  ? 'shrinkToFit'
                  : 'wrapText'),
              '1'));
        }
        ...
        if (verticalAlign != VerticalAlign.Bottom) {
          ...
          childAttributes.add(XmlAttribute(XmlName.parts('vertical'), '$ver'));
        }

        if (horizontalAlign != HorizontalAlign.Left) {
          String hor =
              horizontalAlign == HorizontalAlign.Right ? 'right' : 'center';
          childAttributes.add(XmlAttribute(XmlName.parts('horizontal'), '$hor'));
        }
        if (rotation != 0) {
          childAttributes
              .add(XmlAttribute(XmlName.parts('textRotation'), '$rotation'));
        }

        children.add(XmlElement(XmlName.parts('alignment'), childAttributes, []));
      }

      celx.children.add(XmlElement(XmlName.parts('xf'), attributes, children));
    });
  ...
  void _setColumns(Sheet sheetObject, XmlDocument xmlFile) {
    ...
    if (columnElements.isEmpty) {
      ...
      worksheet.children.insert(index, XmlElement(XmlName.parts('cols'), [], []));
    }
  ...
  XmlElement _createRow(int rowIndex) {
    return XmlElement(XmlName.parts('row'),
        [XmlAttribute(XmlName.parts('r'), (rowIndex + 1).toString())], []);
  }
  ...
  void _setSharedStrings() {
    ...
    [
      ...
      if (shareString.getAttributeNode(value[0]) == null) {
        shareString.attributes.add(XmlAttribute(XmlName.parts(value[0]), value[1]));
      } else {
  ...
  void _setSheetElements() {
    ...
    _excel._sheetMap.forEach((sheetName, sheetObject) {
      ...
      if (sheetFormatPrElement != null) {
        ...
        sheetFormatPrElement = XmlElement(XmlName.parts('sheetFormatPr'), [], []);
        worksheetElement.children.insert(0, sheetFormatPrElement);
      }

      if (defaultRowHeight != null) {
        sheetFormatPrElement!.attributes.add(XmlAttribute(
            XmlName.parts('defaultRowHeight'), defaultRowHeight.toStringAsFixed(2)));
      }
      if (defaultColumnWidth != null) {
        sheetFormatPrElement!.attributes.add(XmlAttribute(
            XmlName.parts('defaultColWidth'), defaultColumnWidth.toStringAsFixed(2)));
      }
